...
class MODELSET:
    ...
    @classmethod
    def TEM(cls, prior=None, time_range=None, device_sttngs=None,
            solver_sttngs=None, solver=None, mdl_unit='res (ohmm)',
            return_rhoa=False, resp_abs=False,
            resp_trafo=None, logTransform=[False, False]):
        """TEM is a class method that generates a MODELSET class object for TEM.
        TODO add comments for TEM!!!
        ...
        method = "TEM"
        if solver == None:
            raise ValueError('please provide a TEM forward solver object (e.g. empymod or simpeg version!)')
        ...
        if mdl_unit == 'res (ohmm)':
            Units = ["\,(\mathrm{m})", "\,(\Omega\,\mathrm{m})"]
            NFull = ["Thickness", "Resistivity_{"]
            NShort = ["\mathrm{thk}_{", r"\rho" + "_{"]
            NGlobal = ["\mathrm{Depth}\,(\mathrm{m})", "\mathrm{Resistivity}\,(\Omega\,\mathrm{m})"]
        elif mdl_unit == 'con (mS/m)':
            Units = ["\,(m)", "\,(\mathrm{mS/m})"]
            NFull = ["Thickness", "Conductivity_{"]
            NShort = ["\mathrm{thk}_{", "\sigma_{"]
            NGlobal = ["\mathrm{Depth}\,(\mathrm{m})", "\mathrm{Conductivity}\,(\mathrm{mS/m})"]
        else:
        ...
        forwardFun = lambda model: frwrd_solver.calc_response(model, mdl_unit=mdl_unit, resp_abs=resp_abs,
                                                              return_rhoa=return_rhoa, resp_trafo=resp_trafo)
...
class PREBEL:
    ...
    def run(self, Parallelization:list=[False, None], RemoveOutlier:bool=False,
            ...
            reduceModels:bool=False, nCompModel=0.9, PriorSampled=None):
        ...
        if (reduceData and not reduceModels):                                  # reduce only data
            ModelsTransform = deepcopy(self.MODELS)
            pca_model = decomposition.PCA(n_components=None)  # no pca
            if type(self.MODPARAM.logTransform[0]) is list:
                ...
                m_h = pca_model.fit_transform(np.log(ModelsTransform))
            else:
                m_h = pca_model.fit_transform(ModelsTransform)
        ...
        elif (not reduceData and reduceModels):                                    # reduce only models
            ...
            if verbose:
                print('only model space reduced with PCA')


        elif (reduceData and reduceModels):                                    # reduce data and models
            pca_model = decomposition.PCA(n_components=nCompModel)
            if type(self.MODPARAM.logTransform[0]) is list:
                if len(self.MODPARAM.logTransform[0]) != len(self.PRIOR):
            ...
            if verbose:
                print('reduced both the data and model space with PCA')

        elif (not reduceData and not reduceModels):                            # reduce nothing
            ModelsTransform = deepcopy(self.MODELS)
            pca_model = decomposition.PCA(n_components=None)  # no pca
            if type(self.MODPARAM.logTransform[0]) is list:
                if len(self.MODPARAM.logTransform[0]) != len(self.PRIOR):
            ...
            raise ValueError('check your inputs for reduceData and reduceModels (booleans only!), nothing done!')
    ...
    def POSTBEL2PREBEL(cls, PREBEL, POSTBEL, Dataset=None, NoiseModel=None,
                       nbMaxPrior:int=1000000, Parallelization:list=[False,None],
                       reduceData:bool=True, nCompData=0.9, verbose=False,
...
class POSTBEL:
    ...
    def run(self, Dataset, nbSamples:int=1000, Graphs:bool=False,
        ...
        if self.MODPARAM.cond is None:
            ...
            # TODO check y_std_, y_mean_ DEPRECATION warning
    ...
    def DataPost(self, Parallelization=[False,None], OtherModels=None, verbose:bool=False):
        ...
        if Parallelization[0]:
            ...
            notComputed = []
            for i in tqdm(range(nbSamples)):  # with progress bar, old: # for i in range(nbSamples):
                # print(i)
                try:
    ...
    def reject_by_threshold(self, threshold, RMSE='relative', NoiseModel=None,
                            Parallelization=[False,None], verbose:bool=False):
        """
        method to reject models based on threshold

        Parameters
        ----------
        threshold : float
            threshold at which samples are rejected. must fit to the selection of RMSE
        NoiseModel : array like. The default is None
            the noise estimation, needs to have the same shape as the data set.
            only necessary if RMSE == 'noise weighted'
        RMSE : string, optional
            which type of root-mean-square-error should be used. The default is 'relative'.
            other options are 'absolute' (in data unts) and error weighted
            (no units, 1 means that the modelled data fit the measured data within the error bounds)
            make sure that the kwarg 'threshold' is selected accordingly.
        Parallelization : list, optional
            parallelization instructions. The default is [False, None].
            [True, None]: parallel runs without pool provided
            [True, pool]: parallel runs with pool (defined bypathos.pools)
                          provided
        verbose : bool, optional
            output progresses messages. The default is False.

        Raises
        ------
        ValueError
            if you do not provide a noise model for the calculation of the RMSE.

        Returns
        -------
        ModelsAccepted : np.array
            Models after threshold rejection.
        DataAccepted : np.array
            Data after threshold rejection.
        ...
        if RMSE == 'absolute':
            ...
            RMS = np.sqrt(np.square(np.divide(diff, (TrueData + self.SAMPLESDATA)/2)).mean(axis=-1)) * 100  # to %

        elif RMSE == 'relative_log':
            diff_log = np.subtract(np.log(TrueData), np.log(self.SAMPLESDATA))
            RMS = np.sqrt(np.square(np.divide(diff_log, np.log(TrueData))).mean(axis=-1)) * 100  # to %

        elif RMSE == 'noise weighted':
            if NoiseModel is None:
                raise ValueError('please provide a noise model with same shape as the data')
            else:
        ...
        mask = RMS < threshold
        if all(mask == False):
            raise ValueError(f"no samples found below the {RMSE} RMS threshold!!!")

        ModelsAccepted = self.SAMPLES[mask]
        DataAccepted = self.SAMPLESDATA[mask]
    ...
    def ShowPostCorr(self, TrueModel=None, OtherMethod=None, OtherInFront=False,
                     alpha=[1, 1], OtherModels=None, color_tm='dodgerblue',
                     color_main='crimson', color_other='coral'):
        ...
        sc_color_main = matplotlib.colors.colorConverter.to_rgba(color_main,
                                                                 alpha=alpha[0])
        sc_color_other = matplotlib.colors.colorConverter.to_rgba(color_other,
                                                                  alpha=alpha[1])
        ...
        for i in range(nbParam):
            for j in range(nbParam):
                ...
                if i == j: # Diagonal
                    ...
                    if OtherInFront:
                        if OtherModels is not None:
                            axs[i,j].hist(OtherModels[:,j],color=color_main,
                                          ec='k', density=True) # Plot the histogram for the given variable
                        else:
                            axs[i,j].hist(self.SAMPLES[:,j],color=color_main,
                                          ec='k', density=True) # Plot the histogram for the given variable
                        if OtherMethod is not None:
                            axs[i,j].hist(OtherMethod[:,j],color=color_other,
                                          ec='k', density=True)
                    ...
                    if TrueModel is not None:
                        axs[i,j].plot([TrueModel[i],TrueModel[i]],
                                      np.asarray(axs[i,j].get_ylim()),
                                      color=color_tm, ls='--', lw=2)
                    ...
                    if i == nbParam-1 and j == nbParam-1:
                        axs[i,j].set_ylabel('pdf ()')
                    ...
                    if OtherModels is not None:
                        axs[i,j].plot(OtherModels[:,j],OtherModels[:,i],marker='o',ls='None',
                                      mec=sc_color_main, mfc='none')
                    else:
                        axs[i,j].plot(self.SAMPLES[:,j],self.SAMPLES[:,i],marker='o',ls='None',
                                      mec=sc_color_main, mfc='none')
                    if TrueModel is not None:
                        axs[i,j].plot(TrueModel[j],TrueModel[i],
                                      'o', color=color_tm)
                    if nbParam > 8:
                        axs[i,j].set_xticks([])
                        axs[i,j].set_yticks([])
                    ...
                    axs[i,j].plot(OtherMethod[:,j],OtherMethod[:,i],marker='o',ls='None',
                                  mec=sc_color_other, mfc='none')
                ...
                if j == nbParam-1:
                    if not(i==j):
                        ...
                        axs[i,j].yaxis.set_label_position("right")
                        axs[i,j].yaxis.tick_right()
                        # axs[i,j].yaxis.set_tick_params(rotation=55, va='bottom', rotation_mode='anchor')
                        pyplot.setp(axs[i,j].yaxis.get_majorticklabels(),
        ...
        for i in range(nbParam):
            for j in range(nbParam):
                if i == j and j != nbParam: # Diagonal
                    # TODO check if this adds labels to first histiogram
                    # if i != 0:
                    axs[i,j].xaxis.set_tick_params(labelbottom=True)
                        # axs[i,j].xaxis.set_tick_params(labeltop=True)
                    if j != nbParam-1:
                        axs[i,j].yaxis.set_tick_params(labelleft=True, rotation=55)
                    axs[i,j].yaxis.set_major_formatter(FormatStrFormatter('%.3f'))
                        # axs[i,j].yaxis.set_tick_params(labelright=True)
    ...
    def ShowPostModels(self, ax=None, TrueModel=None, RMSE='absolute', cmap='viridis',
        ...
        if RMSE is not None:
            ...
            if OtherData is not None:
                ...
                if RMSE == 'absolute':
                    ...
                    fmt = '.1f'
                    # RMS = np.sqrt(np.square(np.divide(diff, (TrueData + self.SAMPLESDATA)/2)).mean(axis=-1)) * 100  # to %
                    RMS = np.sqrt(np.square(np.divide(diff, TrueData)).mean(axis=-1)) * 100  # to %
                elif RMSE == 'relative_log':
                    fmt = '.1f'
                    label = "(log) rel. RMSE (%)"
                    diff_log = np.subtract(np.log(TrueData), np.log(self.SAMPLESDATA))
        ...
        if nbLayer is not None:# If the model can be displayed as layers
            nbParamUnique = int(np.ceil(nbParam/nbLayer))-1 # Number of parameters minus the thickness
            # fig = plt.figure(figsize=[4.5*nbParamUnique, 10])
            if ax == None:
                fig, axes = plt.subplots(1, nbParamUnique, figsize=[5*nbParamUnique, 10],
                    ...
                    for j in range(nbParamUnique):
                        ...
                        if TrueModel is not None:
                            axes[j].step(np.append(TrueMod[j+1][:], TrueMod[j+1][-1]),
                                         ...
                                         where='pre', color='white', label='true model', lw=3)
                        if meanBest is not None:
                            axes[j].step(np.append(meanMod[j+1][:], meanMod[j+1][-1]),
                                         np.append(np.append(0, meanMod[0][:]), maxDepth), lw=3,
                                         where='pre', color='white', label=f'mean of best {meanBest} models')
                        axes[j].invert_yaxis()
                        axes[j].set_ylim(bottom=maxDepth,top=0.0)
                        axes[j].set_xlabel(r'${}$'.format(self.MODPARAM.paramNames["NamesGlobalS"][j+1]),fontsize=14)
                        axes[j].set_ylabel(r'${}$'.format(self.MODPARAM.paramNames["NamesGlobalS"][0]),fontsize=14)
        ...
        # if nbParamUnique > 1:  # removing the ylabel?
        #     for ax in axes.flat:
        #         ax.label_outer()
        ...
        if RMSE is not None:
            ...
            scale = [stats.scoreatpercentile(
                         RMS_scale, a, limit=(np.min(RMS_scale),
                         np.max(RMS_scale)), interpolation_method='lower')
                     for a in np.linspace(0, 100, nb_inter, endpoint=True)]
    ...
    def ShowDataset(self,RMSE:bool=False, Prior:bool=False, cmap='viridis',
        ...
        '''ShowDataset shows the sampled posterior models.
        ...
        if meanBest is not None:
            if abs_mark_sub0:
                ax.plot(xdata, abs(bestMean_data), color='white',
                        label=f'mean of {meanBest} best models')
        ...
        if abs_mark_sub0:
            legend_elements = [Line2D([0], [0], ls='none', marker='s', markersize=5, mec='k',
                                      markerfacecolor='none', markeredgewidth=0.8,
                                      label='sub0 values')]
...
def LoadPOSTBEL(Filename='POSTBEL_Saved.postbel'):
...
def IPR(MODEL:MODELSET, Dataset=None, NoiseEstimate=None, Parallelization:list=[False, None],
        ...
        reduceData:bool=False, n_comp_data:int=5, reduceModels:bool=False, n_comp_model:int=5,
    ...
    It takes as arguments:
        ...
        - Rejection (float=0.9): Maximum quantile of RMSE for rejecting models from the posterior (%/100)
    ...
    It returns possibly 4 elements:
        ...
        - PostbelInit (POSTBEL): a POSTBEL class object containing the posterior model space of the first iteration (before rejection).
    ...
    if PriorSampled is not None:
        Prebel.run(Parallelization=Parallelization, RemoveOutlier=True,
                   reduceData=reduceData, nCompData=n_comp_data,
                   reduceModels=reduceModels, nCompModel=n_comp_model,
                   verbose=verbose, PriorSampled=PriorSampled)
    else:
    ...
    for it in range(nbIterMax):
        ...
        if Mixing is not None:                                     # Iterating
        ...
        if iter == 0:
            Postbel.run(Dataset=Dataset, nbSamples=nbModelsSample,
        ...
        if len(ModelLastIter) > nSamplesConverge: # Testing for convergence (5% probability of false positive)
        ...
        diverge, distance = Tools.ConvergeTest(SamplesA=ModelLastIter,
                                               SamplesB=Postbel.SAMPLES, tol=threshold)
        ...
        if not(diverge):
            ...
            if Graphs:
                NoiseToLastPrebel = Tools.PropagateNoise(Postbel, NoiseLevel=NoiseEstimate,
                                                         verbose=verbose)
                Postbel.KDE.KernelDensity(NoiseError=NoiseToLastPrebel,
                                          RemoveOutlier=True, verbose=verbose)
                Postbel.KDE.ShowKDE(Xvals=Postbel.CCA.transform(Postbel.PCA['Data'].transform(np.reshape(Dataset,(1,-1)))), Noise=NoiseToLastPrebel)
            break
        ...
        if it == 0:
            PostbelInit = deepcopy(Postbel)

        if Rejection > 0:
            RMSE = np.sqrt(np.square(np.subtract(Dataset, PostbelAdd.SAMPLESDATA)).mean(axis=-1))
            RMSE_max = np.quantile(RMSE, 1-Rejection)  # We reject the x/100% worst fit
        ...
        print(f'\n - postbel2prebel with {PostbelAdd.nbSamples} samples')
        Prebel = PREBEL.POSTBEL2PREBEL(PREBEL=Prebel, POSTBEL=PostbelAdd, Dataset=Dataset,
                                       NoiseModel=NoiseEstimate, Parallelization=Parallelization,
                                       reduceData=reduceData, nCompData=n_comp_data,
                                       reduceModels=reduceModels, nCompModel=n_comp_model,
                                       verbose=verbose)
    ...
    if Graphs:
        ...
        for j in range(nbParam):
            ...
            ax.set_title(r'${}$'.format(Prebel.MODPARAM.paramNames["NamesS"][j]))  # NamesFU
    ...
    if not(statsNotReturn):
        return Prebel, Postbel, PrebelInit, PostbelInit, statsReturn
    else:
        return Prebel, Postbel, PrebelInit, PostbelInit


# %% orginal snippets (2022-10)
...
# %% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ SNMR and MAWS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    #def TEMIP(cls, prior=None, timing=None, solver_type='empymod',
              ...
              #ip_modeltype='pelton', unit='res (ohmm)', logTransform=[False, False]):
        ...
        #print('initial Mins and Maxs:\n', Mins, '\n', Maxs)
            ...
            #NShort = ["e_{", r"\rho_{", "mpa_{", "tau_{\phi}_{", "c_{"]
            #NGlobal = ["\mathrm{Depth}\,(m)", "\mathrm{Resistivity}\,(\Omega m)",
...
# Prebel run
    # original version 2022-10
    # def run(self, Parallelization:list=[False, None], RemoveOutlier:bool=False,
    #         reduceModels:bool=False, verbose:bool=False, PriorSampled=None):
    #     """The RUN method runs all the computations for the preparation of BEL1D
