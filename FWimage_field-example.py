...
if __name__=="__main__": # To prevent recomputation when in parallel
    ...
    if FWImage:
        ...
        # # S2 (Mreyen et al. 2021)
        Thickness = np.asarray([0.005, 0.005, 0.025])  # Thickness for the 2 first layers
        ...
        # # S2 (Mreyen et al. 2021)
        real_data_ascii = np.loadtxt(path + 'S2_field-data.ascii')
        source_sw = Source(x=0, y=0, z=0)
        N = 2000  # nr of samples
        T = 0.0005  # dt
        dx = 5  # space between receivers
        array = 120  # length of array (without offset)
        ...
        vmin, vmax, nvel, vspace = 100, 1500, 500, "linear"  # BAL07
